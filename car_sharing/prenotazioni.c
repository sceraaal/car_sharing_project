...
void menuPrenotazioni(Prenotazione **prenota, Veicolo **punt)
{
    ...
    do
    {
        ...
        printf("Benvenuto nel menu prenotazioni!\n");
        ...
        printf("6. Mostrare tutte le prenotazioni (anche quelle non piu' disponibili)\n");
        printf("7. Tornare al menu principale\n");
        printf("Scelta: ");
        scanf("%d", &scelta); 
        ...
        switch(scelta)
        {
            ...
            case 2:
                system("cls");
                {
                   ...
                   *prenota = rimuoviPrenotazione(prenota, codiceDaRimuovere); 
                }
            break;
            ...
            case 4: 
                system("cls");
                {
                    ...
                    float costo = calcolaCostoPrenotazione(*prenota, codiceDaCalcolare); // Chiama la funzione per calcolare il costo della prenotazione
                    Sleep(1000);
            ...
            case 6:
                system("cls");
                mostraPrenotazioniDisponibili(*prenota, *punt); // Chiama la funzione per mostrare le prenotazioni disponibili
            break;
...
void aggiungiPrenotazione(Prenotazione **prenota, Veicolo **punt)
{
    ...
    {
        printf("Non verrà mostrata la lista dei veicoli.\n");
    }
    ...
    {      
        ...
        if(scanf("%d-%d-%d", &dataFine.tm_mday, &dataFine.tm_mon, &dataFine.tm_year) != 3)
        {
...
float calcolaCostoPrenotazione(Prenotazione *prenota, int codiceDaCalcolare)
{
    ...
    printf("COSTO BASE: 50 euro al giorno\n");
    while (corrente != NULL) // Ciclo per calcolare il costo della prenotazione
    {
        if (corrente->codice == codiceDaCalcolare) // Controlla se il codice corrisponde a quello da calcolare
        {
            double durata = difftime(corrente->dataFine, corrente->dataInizio) / (60 * 60 * 24);
            float costoBase = 50.0; // Costo base per giorno
            float costoTotale = costoBase * durata;
            ...
            printf("Durata della prenotazione: %.0f giorni\n", durata); // Mostra la durata della prenotazione
            printf("Codice: %d\n", corrente->codice); // Mostra il codice della prenotazione
            printf("Targa: %s\n", corrente->targa); // Mostra la targa della prenotazione
            printf("Nome: %s\n", corrente->nome); // Mostra il nome della prenotazione
            printf("Cognome: %s\n", corrente->cognome); // Mostra il cognome della prenotazione
            printf("Data inizio: %s", ctime(&corrente->dataInizio)); // Mostra la data di inizio della prenotazione
            printf("Data fine: %s", ctime(&corrente->dataFine)); // Mostra la data di fine della prenotazione
            printf("----------------------------------------------------------\n"); // Separatore per la leggibilità

            premiInvio();
            
            return costoTotale; // Restituisce il costo calcolato
        }
        else
        {
            printf("Prenotazione non trovata.\n"); // Messaggio di errore se la prenotazione non è trovata
        }
...
Prenotazione *rimuoviPrenotazione(Prenotazione **prenota, int codiceDaRimuovere)
{
    ...
    while (nododaeliminare != NULL)
    {
        if (nododaeliminare->codice == codiceDaRimuovere) // Controlla se la targa corrisponde a quella da rimuovere
        {
            ...
            free(nododaeliminare); // Libera la memoria del nodo da eliminare
            printf("Prenotazione rimossa con successo.\n"); // Messaggio di conferma
            Sleep(1000); // Pausa di 2 secondi per dare tempo all'utente di leggere il messaggio
            system("cls"); // Pulisce lo schermo
        }
        else
        {
...
Prenotazione *modificaPrenotazione(Prenotazione **prenota, int codiceDaModificare)
{
    ...
    switch(scelta)
    {
        ...
        case 3:
        {
            ...
            scanf("%d-%d-%d", &dataFine.tm_mday, &dataFine.tm_mon, &dataFine.tm_year); // Legge la data di fine della prenotazione

            dataFine.tm_year -= 1900; // Anno a partire dal 1900
            dataFine.tm_mon -= 1; // Mese a partire da 0
            (*prenota)->dataFine = mktime(&dataFine); // Converte in time_t e assegna
        ...
        default:
            printf("Scelta non valida. Riprova.\n"); // Messaggio di errore se la scelta non è valida
            break;     
    }
    printf("-----------------------------------------------------------\n"); // Separatore per la leggibilità
    premiInvio();
